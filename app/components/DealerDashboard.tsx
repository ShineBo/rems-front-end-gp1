...
type NewProperty = Omit<
  Property,
  "propertyID" | "createdAt" | "updatedAt" | "dealer"
>;
...
export default function DealerDashboard({ dealerID }: { dealerID: number }) {
  ...
  const handleEditProperty = async (propertyData: Property) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/property/${propertyData.propertyID}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      setProperties(
        properties.map((prop) =>
          prop.propertyID === updatedProperty.propertyID
            ? updatedProperty
            : prop
        )
      );
  ...
  const handleDeleteProperty = async (propertyID: number) => {
    ...
    try {
      ...
      setProperties(
        properties.filter((prop) => prop.propertyID !== propertyID)
      );
    } catch (err) {
      ...
      {editingProperty ? (
        ...
        <>
          <div className="flex flex-col md:flex-row justify-between md:items-center space-y-2 md:space-y-0">
            ...
            <div className="flex flex-col text-black md:flex-row gap-2">
              <select
                value={filter}
